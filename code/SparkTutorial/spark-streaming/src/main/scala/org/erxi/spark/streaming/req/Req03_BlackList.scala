...
object Req03_BlackList {
  def main(args: Array[String]): Unit = {
    ...
    ds.foreachRDD(
      rdd => {
        /**
         * rdd. foreach 方法会每一条数据创建连接，foreach 方法是 RDD 的算子，算子之外的代码是在 Driver 端执行，算子内的代码是在 Executor 端执行
         * 这样就会涉及闭包操作，Driver 端的数据就需要传递到 Executor 端，需要将数据进行序列化，数据库的连接对象是不能序列化的。
         * RDD 提供了一个算子可以有效提升效率 : foreachPartition，可以一个分区创建一个连接对象，这样可以大幅度减少连接对象的数量，提升效率
         */
